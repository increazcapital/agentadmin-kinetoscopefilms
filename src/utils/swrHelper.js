/* ============================================================
   Utility: swrHelper.js (Agent Portal)
...
/**
 * Extract userId from stored agent auth data for user-scoped cache keys.
 */
export function getSWRUserId() {
  try {
    const raw = localStorage.getItem('kfpl_agent_auth');
    if (!raw) return 'anon';
    const parsed = JSON.parse(raw);
    const user = parsed?.agent || parsed?.user || parsed?.data || parsed;
