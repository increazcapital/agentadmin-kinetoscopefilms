/* ============================================================
   ...
   Description: Home Screen & Profile KYC Agreement Upload & Download Card for Agent Portal.
                Downloads the exact original agent agreement.docx file,
...
export default function KycAgreementCard({
  ...
  const nameKey = (agentName || 'agent').replace(/\s+/g, '_');

  const [dismissed, setDismissed] = useState(() => {
    return localStorage.getItem(`agent_kyc_banner_dismissed_${nameKey}`) === 'true';
  });
  ...
  if (agreementVerified && !isExpired) {
    ...
    const storageKey = `agent_agreement_verified_time_${nameKey}`;
    if (!verifyTime) {
      ...
      >
        <div style={{ display: 'flex', alignItems: 'center', gap: '14px', flex: 1, minWidth: '260px' }}>
          ...
          <div>
            <h4 style={{ margin: 0, fontSize: '0.98rem', fontWeight: 700, color: '#B45309' }}>
              Congratulations! Agent Service Agreement Verified & Approved 🎉
            </h4>
            <p style={{ margin: '3px 0 0', fontSize: '0.82rem', color: '#D48F00', lineHeight: 1.45 }}>
              Your signed agent agreement and KYC documents have been successfully verified by YieldIQ Team. You can view or download your official contract anytime in your <strong>My Profile</strong> page.
            </p>
          </div>
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
          ...
          <button
            type="button"
            onClick={() => {
              localStorage.setItem(`agent_kyc_banner_dismissed_${nameKey}`, 'true');
              setDismissed(true);
            }}
            style={{
  ...
  /* ── Download Exact Original Agent Agreement .docx File ─ */
  ...
  const handleFileChange = async (e) => {
    ...
    try {
      const res = await apiRequest('/api/agent/documents/agreement', {
        method: 'POST',
        body: formData,
      });
      ...
      if (res && (res.success || res.status === 'success' || res.data?.agreementDocument)) {
        ...
        addToast('Signed agent agreement uploaded successfully for KYC review!', 'success');
        if (onUploadSuccess) onUploadSuccess(uploadedUrl);
      } else {
        addToast(res?.message || 'Failed to upload agent agreement.', 'error');
      }
    } catch (err) {
      addToast(err.message || 'Error uploading agent agreement document.', 'error');
    } finally {
      setUploading(false);
      e.target.value = '';
    }
          ...
          <div>
            <div style={{ display: 'flex', alignItems: 'center', gap: '10px', flexWrap: 'wrap' }}>
              <h3 style={{ margin: 0, fontSize: '1.05rem', fontWeight: 700, color: 'var(--color-text-primary, #0f172a)' }}>
                Official Agent Service Agreement (KYC Requirement)
              </h3>
              {hasAgreement ? (
                <span
                  style={{
                    ...
                    fontSize: '0.72rem',
                    fontWeight: 700,
                    background: agreementVerified ? 'rgba(245, 168, 0, 0.15)' : 'rgba(245, 158, 11, 0.15)',
                    color: agreementVerified ? '#F5A800' : '#D97706',
                    border: `1px solid ${agreementVerified ? '#F5A800' : '#F59E0B'}`,
                  }}
                >
            ...
            <p style={{ margin: '4px 0 0', fontSize: '0.84rem', color: 'var(--color-text-muted, #64748b)', lineHeight: 1.5 }}>
              Download the official Agent Service Agreement (.docx) document, fill in your details, and upload the signed agreement for KYC verification.
            </p>
          </div>
        </div>
