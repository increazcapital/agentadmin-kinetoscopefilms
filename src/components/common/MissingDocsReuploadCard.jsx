...
export default function MissingDocsReuploadCard({ agent, loading = false, onDocUploaded }) {
  ...
  if (!agent.idProofDocument && !agent.aadhaarDocument) {
    missingList.push({ key: 'idProofDocument', label: 'ID Proof (Aadhaar Front / Passport / DL)', shortLabel: 'Aadhaar Front' });
  }
  if (!agent.idProofBackDocument && !agent.aadhaarBackDocument) {
    missingList.push({ key: 'idProofBackDocument', label: 'Aadhaar Card Back Side (Required for Address Proof)', shortLabel: 'Aadhaar Back Side' });
  }
  ...
  const handleFileUpload = async (docKey, file) => {
    ...
    if (!allowed.includes(ext)) {
      toast('Please select a valid PDF, DOCX, or Image file.', 'error');
      return;
    }
    ...
    try {
      ...
      if (docKey === 'agreementDocument') {
        res = await apiRequest('/api/agent/documents/agreement', {
          method: 'POST',
          body: formData,
        });
      } else {
        res = await apiRequest('/api/agent/documents/kyc', {
          method: 'POST',
          body: formData,
        });
      }
      ...
      toast(`${file.name} uploaded successfully! Kinetoscope Films Team will review it shortly.`, 'success', 'Upload Successful');
      onDocUploaded?.(docKey, updatedUrl);
    } catch (err) {
      console.error('Failed to upload document:', err);
      toast(err.message || 'Failed to upload document.', 'error', 'Upload Error');
    } finally {
        ...
        <div style={{
          ...
          <div style={{
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                ...
                <div>
                  ...
                  <p style={{ margin: '3px 0 0', fontSize: '0.8rem', color: '#64748B' }}>
                    Please upload the required document(s) to verify your partner account.
                  </p>
                </div>
              </div>
              <button
            ...
            <div style={{
              ...
              Your profile currently has <strong>{missingList.length}</strong> missing or pending document(s). Upload them below to ensure uninterrupted commission payouts and full KYC verification:
            </div>

            <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
              {missingList.map((doc) => (
                <div
                  key={`modal-agent-${doc.key}`}
                  style={{
                    background: '#FFFFFF',
                ...
                >
                  ...
                  <div>
                    ...
                    />
                    <label
                      htmlFor={`modal-agent-file-${doc.key}`}
                      style={{
                        display: 'inline-flex',
                        alignItems: 'center',
                        justifyContent: 'center',
