...
export default function Header({ isCollapsed, onMenuClick }) {
  ...
  const clearAllNotifications = () => {
    try {
      ...
      apiRequest('/api/agent/notifications/all', {
        method: 'DELETE',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ ids: allIds })
      }).catch((e) => console.error('Failed to clear agent notifications:', e));
    } catch (e) {}
    setNotifications([]);
    setUnreadCount(0);
  };
  ...
  const markAllAsRead = () => {
    try {
      const allIds = notifications.map((n) => n.id);
      apiRequest('/api/agent/notifications/all/read', {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ ids: allIds })
      }).catch((e) => console.error('Failed to mark all agent notifications read:', e));
    } catch (e) {}
  ...
  const markSingleAsRead = (id) => {
    try {
      apiRequest(`/api/agent/notifications/${id}/read`, {
        method: 'PATCH'
