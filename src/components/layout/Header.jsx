...
export default function Header({ isCollapsed, onMenuClick }) {
  ...
  const fetchNotifications = async () => {
    try {
      ...
      if (res && (res.notifications || res.data || Array.isArray(res))) {
        ...
        let lastReadTime = 0;
        try {
          const storedIds = localStorage.getItem('kfpl_agent_read_notifs');
          readIds = storedIds ? JSON.parse(storedIds) : [];
          const storedCleared = localStorage.getItem('kfpl_agent_cleared_notifs');
          clearedIds = storedCleared ? JSON.parse(storedCleared) : [];
          const storedTime = localStorage.getItem('kfpl_agent_notifs_last_read');
  ...
  useEffect(() => {
    ...
    window.addEventListener('agentNotificationsUpdated', fetchNotifications);
    return () => {
      clearInterval(interval);
      window.removeEventListener('storage', loadAgentName);
      window.removeEventListener('focus', fetchNotifications);
      window.removeEventListener('agentNotificationsUpdated', fetchNotifications);
    };
  }, []);
  ...
  const clearAllNotifications = () => {
    try {
      ...
      let clearedIds = [];
      try {
        const stored = localStorage.getItem('kfpl_agent_cleared_notifs');
        clearedIds = stored ? JSON.parse(stored) : [];
      } catch (e) {}
      const updatedCleared = Array.from(new Set([...clearedIds, ...allIds]));
      localStorage.setItem('kfpl_agent_cleared_notifs', JSON.stringify(updatedCleared));
    } catch (e) {}
    setNotifications([]);
    setUnreadCount(0);
  };
  ...
  const markAllAsRead = () => {
    ...
    try {
      ...
      localStorage.setItem('kfpl_agent_notifs_last_read', now.toString());
    } catch (e) {}
      ...
      <div className="kfpl-header-right">
        ...
        <div ref={notifRef} style={{ position: 'relative' }}>
          ...
          >
            ...
            {unreadCount > 0 && (
              <span className="kfpl-header-notif-dot" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '0.625rem', fontWeight: 800, color: '#fff', background: '#e11d48', width: '16px', height: '16px', top: '-2px', right: '-2px', borderRadius: '50%', position: 'absolute' }}>
                {unreadCount}
              </span>
            )}
          </button>
